class ADLayer {
  ...
  _prepareECharts() {
    if (!this._registered) {
      this._echarts.registerCoordinateSystem(
        "openlayers",
        this._getE3CoordinateSystem(this.getMap())
      );
  ...
  _createLayerContainer() {
    ...
    let container = (this._container = ADLayer.Element.createDiv(
      "e_container",
      {
        height: size[1] + "px",
        width: size[0] + "px",
...
ADLayer.Element = {
  ...
  getDomCenter: function(dom, id) {
    ...
    return [
      (parseInt(outer_h) - parseInt(inner_h)) / 2,
      (parseInt(outer_w) - parseInt(inner_w)) / 2
    ];
  }
};
